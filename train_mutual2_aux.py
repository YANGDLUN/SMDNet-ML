...
from KD_loss.Knowledge_Distillation.kd_losses.loss_wrapper import LossWrapper
from KD_loss.TorchDistiller.SemSeg.utils.criterion import CriterionKD
from mymodelnew.KD_loss.Knowledge_Distillation.kd_losses.Pair_wise import CriterionPairWiseforWholeFeatAfterPool
...
#载configs下的配置文件
    ...
    logdir = f'run/{time.strftime("%Y-%m-%d-%H-%M")}-NYU-nmew_MUTUAL2-Ai-kld+cur_up1-eploss(pwloss)'
    ...
    # model._unfreeze_bn()
    model_A.load_pre_sa('/media/maps/shuju/yzy/mymodelnew/toolbox/models/segformer/confg_pth/segformer.b4.512x512.ade.160k.pth')
    # model_A.load_pre_sa('/media/maps/shuju/yzy/mymodelnew/toolbox/models/segformer/confg_pth/segformer.b2.512x512.ade.160k.pth')
    ...
    model_B.load_pre_sa('/media/maps/shuju/yzy/mymodelnew/Backbone/p2t_segmentation/ADE20K/sem_fpn_p2t_l_ade20k_80k.pth')
...
###########################################################################################
    ...
    criterion_pw = CriterionPairWiseforWholeFeatAfterPool(scale=0.5).to(device)
    ...
    criterion_wrapper = LossWrapper(MscCrossEntropyLoss(weight=class_weight), KLDLoss(tau=1),CriterionPairWiseforWholeFeatAfterPool(scale=0.5)).to(device)
    ...
    for ep in range(cfg['epochs']):
        ...
        for i, sample in enumerate(train_loader):
            ...
            with amp.autocast():
                ...
                # lossA_pw = criterion_pw(predict_A[1],predict_B[1].detach()) + criterion_pw(predict_A[2],predict_B[2].detach())
                total_A,_ = criterion_wrapper(predict_A[0],predict_B[0],predict_A[1],predict_B[1],predict_A[2],predict_B[2],label, ep)
                # total_A = total_A + lossA_pw
            ####################################################
            ...
            with amp.autocast():
                predict_B = model_B(image, depth)  ########
                with torch.no_grad():
                    predict_A_detach0 = predict_A[0]
                    predict_A_detach1 = predict_A[1]
                    predict_A_detach2 = predict_A[2]

                # loss_B = criterion(predict_B[0], label)  #######################1
                # lossB_A = criterion_kld(predict_B[0], predict_A_detach0)
                # lossFT_B_A = criterion_AT(predict_B[6],predict_A_detach1)
                # total_B = loss_B
                # total_B = loss_B + lossB_A
                # lossB_pw = criterion_pw(predict_B[1],predict_A_detach1) + criterion_pw(predict_B[2],predict_A_detach2)
                _, total_B = criterion_wrapper(predict_A_detach0,predict_B[0], predict_B[1],predict_A_detach1, predict_B[2],predict_A_detach2, label,ep)
                # total_B = total_B + lossB_pw
